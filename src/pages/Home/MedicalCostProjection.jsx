...
export default function MedicalCostProjectionPage() {
...
const handleUpload = async () => {
  ...
  const generateMCPRedactedReport = async () => {
    ...
    try {
      ...
      localStorage.setItem('mcp_case_id', selectedCaseId);
      ...
      console.log('✅ MCP generation started:', result);

      // Wait a moment for the server to initialize the progress store
      await new Promise(resolve => setTimeout(resolve, 500));

      // Trigger report history refresh using multiple methods
      if (window.refreshReportHistory) {
        console.log('🔄 Refreshing report history after MCP generation...');
        await window.refreshReportHistory();
      }

      // Dispatch custom event for report generation
      window.dispatchEvent(new CustomEvent('reportGenerated', {
        detail: { caseId: selectedCaseId, status: 'PROCESSING' }
      }));
  ...
  return (
    <div className="min-h-screen bg-white p-6">
      <div className="max-w-2xl mx-auto">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-slate-900 mb-2">Medical Cost Projection</h1>
            ...
            {!generatedCaseId && (
              <div className="pt-6 border-t border-slate-200">
                ...
                <p className="text-xs text-slate-500 mt-2 text-center">
                  This will generate a unique Case ID for this patient
                </p>
              </div>
            )}
        ...
        {activeTab === 'report' && (
          <div className="bg-white rounded-2xl p-8 border border-slate-200 shadow-2xl space-y-8">
            <div>
              ...
              <p className="text-slate-600">Enter your Case ID to generate a comprehensive MCP report.</p>
            </div>
            ...
            {reportStatus === 'FAILED' && (
              <div className="bg-red-50 border border-red-200 rounded-xl p-5">
                ...
                <button
                  onClick={generateMCPRedactedReport}
      ...
      {(isUploading || isUploadingExisting || isGenerating) && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-gradient-to-br from-blue-900 to-blue-800 rounded-2xl shadow-2xl p-10 w-full max-w-md text-center border border-blue-700">
            <h2 className="text-2xl font-bold mb-6 text-white">
              {isGenerating ? 'Generating Case ID' : isUploading ? 'Uploading Files' : 'Adding Documents'}
            </h2>
            ...
            <div className="mb-4">
              ...
              <p className="text-sm text-gray-200">
                {isGenerating
                  ? 'Triggering workflow with patient information'
                  : isUploading 
                    ? `Uploading ${files.length} file(s) to the workflow`
                    : `Adding ${existingFiles.length} file(s) to case ${existingCaseId}`
                }
              </p>
            </div>
            ...
            <p className="text-xs text-gray-300 mt-4">
              {isGenerating 
                ? 'Please wait while we generate your case ID...'
